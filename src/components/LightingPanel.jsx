...
import { buildPerKeyColorsCCode, hsvToRgb255, NUM_LEDS } from '../codegen/perKeyColors';
...
export default function LightingPanel({
  ...
  const codeContent = useMemo(() => buildPerKeyColorsCCode(safePerKey, layerCount), [perKeyColors, layerCount]);
  ...
  const [scrollFileSaveStatus, setScrollFileSaveStatus] = useState('');
  const saveScrollToFile = async () => {
    if (!scrollTextFilePath) {
      setScrollFileSaveStatus('No path set — configure one in Settings.');
      setTimeout(() => setScrollFileSaveStatus(''), 3000);
      return;
    }
    try {
      await invoke('write_text_file', { path: scrollTextFilePath, content: scrollCCode });
      setScrollFileSaveStatus('Saved!');
      setTimeout(() => setScrollFileSaveStatus(''), 2000);
    } catch (err) {
      setScrollFileSaveStatus(`Error: ${err}`);
      setTimeout(() => setScrollFileSaveStatus(''), 4000);
    }
  };
  ...
  return (
    <div className={`lighting-panel${compact ? ' lighting-compact' : ''}`}>
      ...
      <div className="td-panel-header">
        <span className="td-panel-title">Lighting</span>
        {status && <span className="td-panel-count">{status}</span>}
      </div>
        ...
        <div className="lighting-center">
          ...
          {subTab === 'global' ? (
            ...
            <div className="pk-body">
              ...
              <div className="pk-actions">
                <button onClick={() => setShowScrollModal(true)}>
                  Generate C code
                </button>
              </div>
            </div>
          ...
          <div className="pk-modal" onClick={e => e.stopPropagation()}>
            <div className="pk-modal-header">
              ...
              <div className="pk-modal-btns">
                ...
                <button className="primary" onClick={saveScrollToFile}>Save to file</button>
                <button onClick={() => setShowScrollModal(false)}>Close</button>
              </div>
            </div>
            <pre className="pk-modal-code">{scrollCCode}</pre>
            <div className="pk-modal-hint">
              {scrollFileSaveStatus
                ? <span className={scrollFileSaveStatus.startsWith('Error') ? 'lighting-status error' : 'lighting-status'}>{scrollFileSaveStatus}</span>
                : scrollTextFilePath
                  ? <span>Will save to <code>{scrollTextFilePath}</code></span>
                  : <span>Set a Scroll Text output path in <strong>Settings</strong> to enable Save to file.</span>
              }
            </div>
          </div>
        </div>
